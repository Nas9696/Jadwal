...
export function TimetableWorkspace() {
  ...
  async function generate() {
    ...
    try { setRun(await projectApi.solve(selected.id,{candidate_count:3,time_limit_seconds:10,seed:0,optimization_profile:profile,...(profile==="custom"?{optimization_weights:customWeights}:{})})); } catch (reason) { showError(reason); } finally { setBusy(false); }
  }
