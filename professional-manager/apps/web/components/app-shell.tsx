...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const loadSchools = useCallback(() => { setupApi.schools().then((rows) => { setSchools(rows); const saved = localStorage.getItem("pm-school"); const selected = rows.find((x) => x.id === saved)?.id ?? rows[0]?.id ?? ""; setSchoolId(selected); if (selected) { localStorage.setItem("pm-school", selected); window.dispatchEvent(new CustomEvent("pm-school-change", { detail: selected })); } }).catch(() => setSchools([])); }, []);
  useEffect(() => {
    loadSchools();
    window.addEventListener("pm-schools-refresh", loadSchools);
    return () => window.removeEventListener("pm-schools-refresh", loadSchools);
  }, [loadSchools]);
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <details className="management-nav"><summary>الإدارة والخيارات المتقدمة</summary><Link href="/">الرئيسية</Link><Link href="/subjects-resources">المواد والموارد</Link><Link href="/imports">الاستيراد</Link><Link href="/advanced/timetables">المشاريع والقواعد</Link><Link href="/substitutions">الغياب والبدلاء</Link><Link href="/reports">التقارير والطباعة</Link></details>
