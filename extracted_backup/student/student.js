...
const TIMING_PRESETS = {
    summer: { label: '����', start: '07:00', period: 45, breakAfter: 3, breakMinutes: 20, count: 7 },
    winter: { label: '����', start: '07:30', period: 45, breakAfter: 3, breakMinutes: 20, count: 7 },
    ramadan: { label: '������', start: '09:00', period: 35, breakAfter: 3, breakMinutes: 10, count: 6 },
    custom: { label: '����', start: '07:00', period: 45, breakAfter: 3, breakMinutes: 20, count: 7 }
};
...
function buildSectionsForState(target) {
    ...
    const numbers = { '�����': '1', '������': '2', '������': '3', '������': '4', '������': '5', '������': '6', '����� �����': '1', '������ �����': '2', '������ �����': '3', '����� �����': '1', '������ �����': '2', '������ �����': '3' };
    const shortNames = { '�����': '���', '������': '����', '������': '����', '������': '����', '������': '����', '������': '����', '����� �����': '��� �����', '������ �����': '���� �����', '������ �����': '���� �����', '����� �����': '��� �����', '������ �����': '���� �����', '������ �����': '���� �����' };
...
function getTimingSlots() {
    ...
    for (let i = 1; i <= Number(state.timing.count || 7); i++) {
        ...
        slots.push({ number: i, label: `����� ${i}`, start, end });
...
function fillTeacherSelect() {
    const options = [{ value: '', label: '���� �����' }, ...state.teachers.map(t => ({ value: t.name, label: `${t.name} - ${t.subject}` }))];
...
function renderHeader() {
    ...
    byId('header-student').textContent = state.studentName ? `������: ${state.studentName}` : '��� ��� ������ �� ���� ������';
    ...
    byId('summary-timing').textContent = TIMING_PRESETS[state.timingMode]?.label || '����';
}
...
function renderGrid() {
    ...
    DAYS.forEach(day => {
        ...
        slots.forEach(slot => {
            ...
            html += `<td>${lesson ? lessonCard(lesson) : `<button class="empty-slot" onclick="quickPrepare('${day}', ${slot.number})">�����</button>`}</td>`;
        });
        html += '</tr>';
    });
    html += '</tbody></table>';
...
function lessonCard(lesson) {
    return `
        <div class="lesson-card">
            ...
            <button onclick="deleteSession('${lesson.id}')" title="���"><i data-lucide="x"></i></button>
        </div>
    `;
}
...
function renderDashboard() {
    ...
    byId('today-list').innerHTML = todayLessons.length ? todayLessons.map(item => `
        <div class="mini-card">
            ...
            <span>${item.teacher || '���� ����'} - ${window.toArabicDigits(item.start)} ��� ${window.toArabicDigits(item.end)}</span>
        </div>
    ...
    byId('urgent-tasks').innerHTML = upcoming.length ? upcoming.map(taskCard).join('') : '<div class="empty-state">�� ���� ���� �����.</div>';
}

function renderTasks() {
    const tasks = [...state.tasks].sort((a, b) => String(a.date).localeCompare(String(b.date)));
    byId('tasks-container').innerHTML = tasks.length ? tasks.map(taskCard).join('') : '<div class="empty-state">�� ���� ������ �� ��������.</div>';
    if (window.lucide) window.lucide.createIcons();
}

function taskCard(task) {
    const labels = { homework: '����', exam: '������', review: '������' };
    return `
        <div class="task-card ${task.type}">
            <div>
                <span>${labels[task.type] || '����'}</span>
                <strong>${task.title}</strong>
                <small>${task.subject} - ${task.date || '���� �����'} ${task.time ? window.toArabicDigits(task.time) : ''}</small>
            </div>
            <button onclick="deleteTask('${task.id}')" title="���"><i data-lucide="trash-2"></i></button>
        </div>
    `;
}
...
function renderTeachers() {
    ...
    byId('teachers-list').innerHTML = state.teachers.length ? state.teachers.map(teacher => `
        <div class="teacher-card">
            ...
            <button onclick="deleteTeacher('${teacher.id}')" title="���"><i data-lucide="trash-2"></i></button>
        </div>
    `).join('') : '<div class="empty-state">�� ��� ����� ������ ���.</div>';
}
...
function updateTimer() {
    ...
    if (status.current) {
        ...
        byId('next-subject').textContent = status.next ? `������: ${status.next.subject}` : '��� ��� ��� �����';
        ...
        byId('current-subject').textContent = '������� / ������';
        byId('next-subject').textContent = `������: ${status.next.subject}`;
        byId('countdown').textContent = window.formatDuration(status.remainingSeconds);
        byId('timer-progress').style.strokeDashoffset = 377;
    } else {
        byId('current-subject').textContent = '����� ����� �������';
        byId('next-subject').textContent = '���� ����� ������ ����';
