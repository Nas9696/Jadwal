...
const materialVisuals = {
    '�������': { emoji: '?', icon: 'calculator', color: '#2563eb' },
    '����': { emoji: '??', icon: 'book-open-text', color: '#dc2626' },
    '����': { emoji: '??', icon: 'microscope', color: '#059669' },
    '�������': { emoji: '??', icon: 'landmark', color: '#047857' },
    '�������': { emoji: '??', icon: 'languages', color: '#7c3aed' },
    '�����': { emoji: '??', icon: 'monitor', color: '#0891b2' },
    '������ ������': { emoji: '??', icon: 'heart-handshake', color: '#65a30d', short: '�. ������' },
    '���������': { emoji: '??', icon: 'globe-2', color: '#ca8a04' },
    '����': { emoji: '??', icon: 'palette', color: '#db2777' },
    '������': { emoji: '??', icon: 'atom', color: '#4f46e5' },
    '������': { emoji: '??', icon: 'flask-conical', color: '#0d9488' },
    '�����': { emoji: '??', icon: 'dna', color: '#16a34a' },
    '������': { emoji: '??', icon: 'pen-line', color: '#9333ea' },
    '�����': { emoji: '?', icon: 'coffee', color: '#64748b' }
};
...
function parseArabicNum(str) {
    ...
    const englishDigits = String(str).replace(/[0123456789]/g, d => '0123456789'.indexOf(d));
    ...
    if (isMinisterial) {
        if (isEnabled('merge_weeks') && isEnabled('week2_enabled')) {
            ...
            if (isEnabled('show_week_box')) {
                ...
                wb1.innerHTML = `<div class="inline-block bg-[#2a8da8] text-white px-10 py-2 rounded-lg font-black text-base shadow">${week1Text} � ${week2Text}</div>`;
...
function buildWeekRows(daysCountRaw, startDayRaw, startMonthRaw, startDayName, oldRows) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        rows.push({
            ...
            test_type: oldRow.test_type || '������',
            material: oldRow.material || ''
        });
        currentD += (daysSequence[(startIdx + i) % 5] === '������') ? 3 : 1;
...
function renderTableMultiColumn(head, body) {
    const headerRow = document.createElement('tr');
    ['�����', '�������', ...state.classNames].forEach((label, index) => {
        const th = document.createElement('th');
        th.className = 'p-3 border-2 border-slate-900';
        th.textContent = label;
        if (label.length > 15) {
    ...
    state.tableRows.forEach((row, rIdx) => {
        ...
        row.subjects.forEach((sub, sIdx) => {
            ...
            addBtn.innerHTML = '<i data-lucide="plus" class="w-3.5 h-3.5"></i><span>����� ����</span>';
...
function renderTableListMode(head, body) {
    ...
    state.tableRows.forEach((row, rIdx) => {
        ...
        const classesCell = createInputCell(rIdx, 'classes', '���� ������...');
...
function autoFitPrint() {
    ...
    // 2. ���� ������ A4 ������ (��� ��� �������)
    // @page margin: 6mm � padding: 4mm ? ����� = 10mm �� �� ���� = 20mm
...
function shareWhatsApp() {
    ...
    const renderTableToWhatsApp = (tableRows, weekTitle) => {
        ...
        tableRows.forEach(row => {
            msg += `??????????????\n`;
            msg += `?? *�����:* ${row.day}\n`;
            msg += `??? *�������:* ${isArabic ? toArabicDigits(row.date) : row.date}\n\n`;

            if (isMinisterial) {
                if (row.material) {
                    const materialVisual = getMaterialVisual(row.material);
                    msg += `??? ${row.classes || stageClassesText[state.currentStage] || ''} : ${materialVisual.emoji} ${row.material} (${row.test_type || ''})\n`;
                }
            } else {
